...
import io.github.jadarma.steko.core.Key
...
class SinkTest : FunSpec({
    ...
    test("Only writes to bit-masked bits") {
        ...
        val blackSteko = StekoAlgorithm.createFor(blackImage.copy().pixels, lsbKey)
        val whiteSteko = StekoAlgorithm.createFor(whiteImage.copy().pixels, lsbKey)

        val (blackSink, blackSource) = StekoSink(blackSteko) to StekoSource(blackSteko)
        val (whiteSink, whiteSource) = StekoSink(whiteSteko) to StekoSource(whiteSteko)

        val capacity = withClue("Image capacity not calculated correctly") {
            blackSteko.capacity
                .shouldBe(whiteSteko.capacity)
                .shouldBe(36)
        }
        ...
        withClue("Modified pixels outside of bitmask") {
            blackSteko.pixels.forEachIndexed { index, value ->
                val original = blackImage.pixels[index]
                original shouldBe 0
                value xor original shouldBe blackSteko.bitmask
            }
            whiteSteko.pixels.forEachIndexed { index, value ->
                val original = whiteImage.pixels[index]
                original shouldBe 0xFFFFFFFF.toInt()
                value xor original shouldBe whiteSteko.bitmask
            }
        }
    }
    ...
    test("Preserves bit order") {
        ...
        val steko = StekoAlgorithm.createFor(image.pixels, key)

        StekoSink(steko).write(buffer, 1)
        buffer.size shouldBe 0L
        StekoSource(steko).readAtMostTo(buffer, 1) shouldBe 1L
