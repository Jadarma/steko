...
class RandomTest : FunSpec({
    ...
    test("The image pixel order is the same when using the same key.") {
        ...
        val shuffledByA = StekoAlgorithm.createFor(image.pixels, keyA).pixelOrder
        val shuffledByB = StekoAlgorithm.createFor(image.pixels, keyB).pixelOrder
        val shuffledByC = StekoAlgorithm.createFor(image.pixels, keyC).pixelOrder
