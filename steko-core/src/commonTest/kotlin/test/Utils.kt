...
internal suspend fun hidingTest(
    ...
    StekoAlgorithm.createFor(actualImage.pixels, key).hide(
...
internal suspend fun showingTest(
    carrier: Resource,
    hexKey: Resource,
    expected: StekoPayload?,
) {
    val carrierImage = Image.decodeFromRgba(carrier)
    val key = Key(hex = hexKey.readText().trimEnd())

    val result = StekoAlgorithm.createFor(carrierImage.pixels, key).show()
