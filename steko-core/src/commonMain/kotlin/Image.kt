...
import io.github.jadarma.steko.core.internal.StekoAlgorithm
import io.github.jadarma.steko.core.internal.cborFormat
import io.github.jadarma.steko.core.internal.encodeToByteArray
...
public class Image(
     ...
     */
    public suspend fun hide(key: Key, payload: StekoPayload, noise: Boolean = true): Image = apply {
        StekoAlgorithm.createFor(pixels, key).hide(
     ...
     */
    public suspend fun show(key: Key): StekoPayload? {
        val (data, isRaw) = StekoAlgorithm.createFor(pixels, key).show() ?: return null
    ...
    public suspend fun <T : Any> show(key: Key, convert: (ByteArray) -> T): T? {
        val data = StekoAlgorithm.createFor(pixels, key).show()?.first ?: return null
        return convert(data)
    }
