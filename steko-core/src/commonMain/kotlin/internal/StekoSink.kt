...
/**
 * A sink that can write bytes to pixel data using the [steko] algorithm.
 * Note that this source does *not* perform encryption or prepend headers, it will only write the data as-is.
 */
internal class StekoSink(private val steko: StekoAlgorithm) : RawSink {

    private var index = 0L

    override fun write(source: Buffer, byteCount: Long) {
        if (index >= steko.capacity || byteCount < 0) return
        @Suppress("unused")
        for (i in 0 until minOf(byteCount, steko.capacity - index)) {
            writeByte(source.readByte())
        }
    }

    private fun writeByte(value: Byte) = with(steko) {
